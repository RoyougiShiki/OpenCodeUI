...
class ThemeStore {
  ...
  setPermissionDialogCollapsed(enabled: boolean) {
    if (this.state.permissionDialogCollapsed === enabled) return
    this.state = { ...this.state, permissionDialogCollapsed: enabled }
    localStorage.setItem(STORAGE_KEY_PERMISSION_DIALOG_COLLAPSED, String(enabled))
    this.emit()
  }
...
class ThemeStore {
  ...
  setImmersiveMode(enabled: boolean) {
    ...
    this.state = {
      ...
      toolCardStyle: enabled ? 'compact' : 'classic',
      compactInlinePermission: enabled,
      permissionDialogCollapsed: enabled,
    }
    localStorage.setItem(STORAGE_KEY_IMMERSIVE_MODE, String(enabled))
    localStorage.setItem(STORAGE_KEY_INLINE_TOOL_REQUESTS, String(enabled))
    localStorage.setItem(STORAGE_KEY_DESCRIPTIVE_TOOL_STEPS, String(enabled))
...
export function importThemeBackup(raw: unknown): void {
 ...
 (feat(settings): add tool output collapse setting, merge permission toggles)
